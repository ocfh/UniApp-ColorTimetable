...
import TimetableContent from '~/components/timetable/DayContent.vue'
...
const currentDate = new Date();const date = `${currentDate.getFullYear()}${String(currentDate.getMonth() + 1).padStart(2, '0')}${String(currentDate.getDate()).padStart(2, '0')}`;
  uni.navigateTo({
    url: `/pages/index/courses?${date ? `date=${date}` : 'no'}`,
  })
}
  ...
  <UBasePage>
    ...
    >
      <div :style="{ 'padding-top': `${statusBarHeight}px`, 'height': `${customBarHeight - statusBarHeight}px` }">
		...
		<a style="text-decoration: none; float: left; font-size: 14px; display: inline-block; padding: 4px 8px; font-weight: normal; border-radius: 100px; line-height: initial; color: rgb(255, 255, 255); margin-right: auto; background: linear-gradient(rgba(255, 255, 255, 0.1), rgba(240, 242, 244, 0.1)); box-shadow: rgba(255, 255, 255, 0.4) 0px 2px 2px, rgb(255, 255, 255) 0px 0px 0px 1px inset;" @click="clearSelectedClass">切换班级</a>
		<a style="font-size: 18px;display: inline-flex;margin-right: auto;">{{  !isStart ? '未开学':`第${currentWeekIndex + 1}周`}}</a>
          <div 
...
>
            <a style="float: right; font-size: 14px; text-decoration: none; display: inline-block; padding: 4px 8px; border-radius: 100px; color: rgb(255, 255, 255); line-height: initial; background: linear-gradient(rgba(255, 255, 255, 0.1), rgba(240, 242, 244, 0.1)); box-shadow: rgba(255, 255, 255, 0.4) 0px 2px 2px, rgb(255, 255, 255) 0px 0px 0px 1px inset"  @click="handleCreateCourse">进入周表</a>
          </div>
        </div>
      </div>
    </div>
